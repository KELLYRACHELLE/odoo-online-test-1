...
class SalWizard(models.TransientModel):
    ...
    session_student_ids = fields.Many2many(comodel_name='res.partner',
                                           ...
                                           help='these are the student currenty in the Session')
        ...
        if session_product_id:
            for student in self.student_ids:
                order_id = self.env['sale.order'].create({
                    ...
                    'order_line': [(O, 0, {'product_id': session_product_id.id, 'prie_unit' :
                                            self.session_id.total_price})]
                })